...
def extractor1(words = {}, sid=0,aspect_terms=[]):				#sid = sentence id, and words = aspect terms 
	inner={}
	#print words
	for j in aspect_terms:						# one by one aspect terms theeskuntam
	    lit=[]
	 #   print j						#j is the aspect term
	    flag=0
	  #  print sid,j
	    p =  words[j]				#parser lo nunchi aa aspect term theeskuntam, from that sid
	   # print "pp;;",p
	    if words[j]['pos_tag'] in noun:		#then if aspect term is noun go inside
		#print 'yipppeeeeeee'
		for i in p:					#iterate through its values
		    if i != 'pos_tag':				#if key is not 'pos_tag' go inside
			#print p[i]
